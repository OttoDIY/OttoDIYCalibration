...
public class Otto_Servo_Calibration extends PApplet {
...
public int ax(float cx) {
  return PApplet.parseInt(cx * scale + width/2);
}

public int ay(float y) {
  return PApplet.parseInt(y * scale);
}

public int as(float s) {
  return PApplet.parseInt(s * scale);
}
...
public void draw(){
  if (prevWidth != width || prevHeight != height) {
    prevWidth = width;
    prevHeight = height;
    scale =  PApplet.parseFloat(height) / ORIGINAL_HEIGHT;
  }
...
public void COM_select() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Otto_Servo_Calibration" };
    if (passedArgs != null) {
